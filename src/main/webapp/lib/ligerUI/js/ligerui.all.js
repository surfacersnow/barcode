...
(function ($)
{
    ...
    liger.controls.Input.ligerExtend(liger.core.UIComponent, {
        ...
        {
            this.set({ width: width, height: height});//height + 2-->height，表格添加行时高度一致2015-11-8 ldz
        }
    });
        ...
        var defaultOp = {
            ...
            getValue: function (editor, editParm)
            {
                ...
                if (editor.getValue)
                {
                    ...
                    if (field && field.editor && field.editor.isRef && this.getText(editor, editParm))//2016-1-28 ldz 修改editor.getText-->this.getText(editor, editParm)
                    {
                        value = [value, this.getText(editor, editParm)];//2016-1-28 ldz 修改editor.getText()-->this.getText(editor, editParm)
                    }
            ...
            { 
                ...
                if (editor.setValue)
                {
                    ...
                    if(editor.type=="Select2"){//ldz 2016-1-28 添加对select2插件的支持
                    	$(editor.element).val(value).trigger("change");
                    }
            ...
            getText: function (editor, editParm)
            {
                ...
                if (editor.getText)
                {
                    ...
                    if (text) return text; 
                }else if(editor.type=="Select2"){//ldz 2016-1-28 添加对select2插件的支持
                	var text = $(editor.element).find("option:selected").html();
                	if (text) return text;
                }
            },
            //设置文本信息到控件去
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        parms: [],                         //提交到服务器的参数
        // wws modify
        postData:null,                    //提交到服务器的 ajax 的 data
        sqlid:null,                       //提交后台的SQLID
        columns: [],                          //数据源
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        loadServerData: function (param, clause, sourceType)
        {
            ...
            // wws modify
            // url 不为 false 时通过 ajax 传递 json格式 data 数据
            if(p.url){
            	var param = new Object();
            	//若定义了查询参数，取定义参数
            	if (p.postData)
            	param.data =p.postData;
            	//设置当前页
            	param.cp=p.newPage;
            	//设置页容量
            	param.ps=p.pageSize;
        ...
        _getTotalCellContent: function (column, data)
        {
            ...
            if (column.totalSummary)
            {
                ...
                else if (column.totalSummary.type && info)
                {
                    ...
                    if (isExist('Nsum'))
                        totalsummaryArr.push("<div>" + info.sum.toFixed(2) + "</div>");
                    if (isExist('count'))
    ...
    $.ligerui.controls.Panel.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setChildren: function(children)
        {
            ...
            var PluginNameMatchs  = 
            {
                ...
                "tab":"ligerTab",
                "select2":"ligerSelect2"//ldz 自定义扩展
            }; 
...
(function ($)
{
    ...
    $.ligerui.controls.PopupEdit.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            //开关
            g.link = $('<div class="l-trigger"><div class="l-trigger-icon"><span class="glyphicon glyphicon-option-horizontal"></span></div></div>');//ldz 2016-1-21 修改<div class="l-trigger"><div class="l-trigger-icon"></div></div>
            //外层
...
(function ($)
{
    $.fn.ligerSelect2 = function ()
    {
        return $.ligerui.run.call(this, "ligerSelect2", arguments);
    };

    $.fn.ligerGetSelect2Manager = function ()
    {
        return $.ligerui.run.call(this, "ligerGetSelect2Manager", arguments);
    };

    $.ligerDefaults.Select2 = {
        onChangeValue: null,
        onMouseOver: null,
        onMouseOut: null,
        onBlur: null,
    ...
    $.ligerui.controls.Select2 = function (element, options)
    {
        $.ligerui.controls.Select2.base.constructor.call(this, element, options);
    };

    $.ligerui.controls.Select2.ligerExtend($.ligerui.controls.Input, {
        __getType: function ()
        {
            return 'Select2'
        },
        ...
        _init: function ()
        {
            $.ligerui.controls.Select2.base._init.call(this);
            var g = this, p = this.options;
            if (!p.width)
            {
                p.width = $(g.element).width();
        ...
        _render: function ()
        {
            ...
            g.inputText = $(this.element);
            //外层
            g.wrapper = g.inputText.wrap('<div class="l-text l-select2"></div>').parent();
            g.inputText.attr({
            	id:p.name,
            	name:p.name
            });
            //初始化select2插件
            bindSelect(p.name,p.data,[],false);
            g.wrapper.append('<div class="l-text-l"></div><div class="l-text-r"></div>');
            if (!g.inputText.hasClass("l-text-field"))
                g.inputText.addClass("l-text-field");
            this._setEvent();
        ...
        _setEvent: function ()
        {
            ...
            g.inputText.bind('blur.select2', function ()
            {
                g.trigger('blur');
                g.checkNotNull();
                g.formatValue();
            ...
            { 
            	...
            	g.trigger('selected');
                g.checkNotNull();
                g.inputText.val(data);
                g.wrapper.removeClass("l-text-focus");
            })
            .change(function ()
            {
                g.trigger('selected', this.value);
            });
            g.wrapper.hover(function ()
            {
                g.trigger('mouseOver');
